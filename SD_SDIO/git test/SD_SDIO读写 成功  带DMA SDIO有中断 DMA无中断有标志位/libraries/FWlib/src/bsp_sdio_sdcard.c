...
void SD_DeInit(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;		//双向数据线，因此不予配置模式
...
static void GPIO_Configuration(void)
{
	...
	/* SD卡  SDIO接口的配置 共有六条线需要配置 DATA0-3 CLK  CMD  */
	//PC8 PC9 PC10 PC11 为DATA0-3      PC12为时钟端CLK   均需复用推挽输出   
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12;//
	GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_AF_PP;
	GPIO_Init(GPIOC,&GPIO_InitStructure);//大小写字母要分清，否则出错		
	
	
	//D2  CMD  接收主机命令  。模式速度先不予配置
...
SD_Error SD_Init(void)
{
  ...
  SDIO_InitStructure.SDIO_ClockDiv = SDIO_TRANSFER_CLK_DIV;			//首先将卡降速，无论高低速均可响应

	/*上升沿采集数据 */
  SDIO_InitStructure.SDIO_ClockEdge = SDIO_ClockEdge_Rising;

	/* 时钟频率若超过24M,要开启此模式 */
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable; 			//取消旁路，时钟旁路不使能
	
	/* 若开启此功能，在总线空闲时关闭sd_clk时钟 */
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;		//省电模式取消
  ...
  if (errorstatus == SD_OK)
  {
			/* 最后为了提高读写，开启4bits模式 */
			printf(" SD卡已选择成功.\r\n");
    errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);			//开启四位总线传输模式
  }  
		printf(" 已开启4位数据传输模式 .\r\n");
		return(errorstatus);
}
...
/********************************************************************************************************/   
  ...
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_GO_IDLE_STATE;			//复位空闲状态,发送0号命令
...
/********************************************************************************************************/
   ...
   * CMD Response: R7 
	 */
	 /* 接收到命令sd会返回这个参数 */
  SDIO_CmdInitStructure.SDIO_Argument = SD_CHECK_PATTERN;			//检查卡的类型参数
	
  SDIO_CmdInitStructure.SDIO_CmdIndex = SDIO_SEND_IF_COND;		//发送8号命令
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;	//短响应 
	
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;			 				
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;			//使能状态机
  SDIO_SendCommand(&SDIO_CmdInitStructure);						//发送出去命令
  ...
  if (errorstatus == SD_OK)	  	
  {
    ...
    SDType = SD_HIGH_CAPACITY;	
		printf(" 已确定为符合2.0协议的SD卡  \r\n");
  }
  else	/* 无响应，说明是1.x的或mmc的卡 */
  {
    /* 发命令 CMD55 */	  		//发应用层命令必须先发CMD55
    ...
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&SDIO_CmdInitStructure);
    errorstatus = CmdResp1Error(SD_CMD_APP_CMD);			//获取卡响应
		printf(" 2.0协议无响应，可能为1.0卡或者卡不存在！  \r\n");
  }
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF)
  {
	...
	/* 判断卡的类型 */
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)			//如果不是SD卡
  {
    /* Send CMD2 ALL_SEND_CID 发送一个2号命令
		 * 响应：R2，对应CID寄存器
		 */
    SDIO_CmdInitStructure.SDIO_Argument = 0x0;
...
/********************************************************************************************************/
  ...
  {
    /* Send CMD3 SET_REL_ADDR with argument 0 发送CMD3的命令  。要求卡发布RCA  相对地址
...
/********************************************************************************************************/
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    ...
    /* Send CMD9 SEND_CSD with argument as card's RCA 发布一个9号命令  获取CSD
...
SD_Error SD_ReadMultiBlocks(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToSDIO;//传输方向   从卡到控制器
...
SD_Error SD_WriteMultiBlocks(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
 ...
 /*********************************************************************************/
  ...
  SDIO_DataInitStructure.SDIO_DataBlockSize = (uint32_t) 9 << 4;
  SDIO_DataInitStructure.SDIO_TransferDir = SDIO_TransferDir_ToCard;			//传输方向由控制器到卡 
  SDIO_DataInitStructure.SDIO_TransferMode = SDIO_TransferMode_Block;			//传输的模式是按块读写
  SDIO_DataInitStructure.SDIO_DPSM = SDIO_DPSM_Enable;
  SDIO_DataConfig(&SDIO_DataInitStructure);
